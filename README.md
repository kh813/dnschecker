...
2.  **DNS設定の入力方法の選択**:
    *   **直接入力**: Value-domainのDNS設定をコピーし、ターミナルに直接貼り付け、入力終了後に空行でEnterを押してください。
...
ソースコードから実行ファイルをビルド（コンパイル）する方法です。

### ビルド方法

`make`コマンドを使用して、各種OS向けの実行ファイルをビルドできます。

| コマンド      | 説明                                               |
| ------------- | -------------------------------------------------- |
| `make`        | ヘルプを表示します                                 |
| `make build`  | 現在のOS向けの実行ファイルをビルドします           |
| `make all`    | Windows, Mac, Linux向けの実行ファイルを全てビルドします |
| `make win`    | Windows (x64) 向けの実行ファイルをビルドします     |
| `make mac`    | Mac (Apple Silicon) 向けの実行ファイルをビルドします |
| `make lin`    | Linux (x64) 向けの実行ファイルをビルドします       |
| `make clean`  | ビルドで生成されたファイルを削除します             |
